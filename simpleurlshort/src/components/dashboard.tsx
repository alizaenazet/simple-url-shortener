...
const BASE_API_URL = "https://a0c5dfb1-5418-447c-8214-111547ff8b60.mock.pstmn.io/urls" 
const SHORT_LINK_BASE_URL = "https://yourdomain.com/" 
...
const Dashboard = () => {
  ...
  const fetchUserLinks = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_API_URL}/urls?${queryParams}`, {
        method: "GET",
        headers: {
  ...
  }, [currentPage, itemsPerPage, sortBy, sortOrder, filter, navigate])
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestBody = {
        ...
        customShort: createForm.customShort || undefined, // Send undefined if empty
        expiresInDays: createForm.expiresInDays
      }

      const response = await fetch(`${BASE_API_URL}/urls`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteLink = async (shortCode: string) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`${BASE_API_URL}/urls/${shortCode}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
